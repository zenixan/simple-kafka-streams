...
trait IterativeFunctions[K, V]
    extends org.eu.fuzzy.kafka.streams.functions.IterativeFunctions[K, V] {

  /**
   * Performs an action for each input record.
   *
   * This is a stateless record-by-record operation that triggers a side effect
   * (such as logging or statistics collection) and returns an unchanged stream.
   *
