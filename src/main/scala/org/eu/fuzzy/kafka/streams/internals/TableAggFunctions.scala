 ...
 */
private[streams] class TableAggFunctions[K, V](
  ...
  override def aggregate[VR](
    ...
    val newTable = internalTable.aggregate(
  ...
  override def aggregate[VR](initializer: () => VR, adder: (K, V, VR) => VR, subtractor: (K, V, VR) => VR)
                            (implicit serde: ValueSerde[VR]): KTable[K, VR] =
    aggregate(initializer, adder, subtractor, storeOptions(topic.keySerde, serde))

  override def reduce(adder: (V, V) => V, subtractor: (V, V) => V, options: Options[K, V]): KTable[K, V] = {
    val newTopic = toTopic(options)
    val newTable = internalTable.reduce(
      adder.asReducer(topic, errorHandler),
  ...
  override def count(options: Options[K, Long]) = aggregate(
    () => 0L,
