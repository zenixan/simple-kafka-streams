...
trait AggregateFunctions[K, V] extends org.eu.fuzzy.kafka.streams.functions.AggregateFunctions[K, V] {
   ...
   *  - When a record key is received for the first time, the initializer is called (and called before the aggregator).
   *
   ...
   */
  def aggregate[VR](initializer: () => VR, aggregator: (K, V, VR) => VR)(implicit serde: ValueSerde[VR]): KTable[K, VR]
   ...
   */
  def aggregate[VR](initializer: () => VR, aggregator: (K, V, VR) => VR, options: KTable.Options[K, VR]): KTable[K, VR]
   ...
   * The behavior of this operation is:
   *  - Records with `null` keys or values are ignored.
   *  - The reducer will be called with the last reduced value and new value if a record with a non-null value is received.
   *
