 ...
 */
@implicitNotFound("Unable to find a serializer for the value type ${T}")
trait ValueSerde[T] extends Serde[T] with Configurable
...
object ValueSerde {
   ...
   */
  def apply[T](serde: Serde[T]): ValueSerde[T] = ValueSerde(serde.serializer, serde.deserializer)
   ...
   */
  def apply[T](kafkaSerializer: Serializer[T], kafkaDeserializer: Deserializer[T]): ValueSerde[T] = new ValueSerde[T]() {
