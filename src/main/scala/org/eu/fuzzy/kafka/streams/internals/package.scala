...
package object internals {
  ...
  private[streams] implicit class RichJoiner[V, VO, VR](f: (V, VO) => VR) {
    ...
    @inline def asInnerJoiner[K](topic: KTopic[K, V], handler: ErrorHandler): ValueJoiner[V, VO, VR] =
      (value1, value2) => Try(f(value1, value2)).recover(handler.handle(topic, InnerJoinOperation, value1, value2)).get

    /** Wraps a left joiner function by the given error handler. */
    @inline def asLeftJoiner[K](topic: KTopic[K, V], handler: ErrorHandler): ValueJoiner[V, VO, VR] =
      (value1, value2) => Try(f(value1, value2)).recover(handler.handle(topic, LeftJoinOperation, value1, value2)).get

    /** Wraps an outer joiner function by the given error handler. */
    @inline def asOuterJoiner[K](topic: KTopic[K, V], handler: ErrorHandler): ValueJoiner[V, VO, VR] =
      (value1, value2) => Try(f(value1, value2)).recover(handler.handle(topic, OuterJoinOperation, value1, value2)).get
  }
  ...
  private[streams] implicit class RichReducer[V](f: (V, V) => V) {
    ...
    @inline def asReducer[K](topic: KTopic[K, V], handler: ErrorHandler): Reducer[V] =
      (aggregate, value) => Try(f(aggregate, value)).recover(handler.handle(topic, ReduceOperation, aggregate, value)).get
  }
  ...
  private[streams] implicit class RichAggregator[K, V, VR](f: (K, V, VR) => VR) {
    ...
    @inline def asAggregator(topic: KTopic[K, V], handler: ErrorHandler): Aggregator[K, V, VR] =
      (key, value, aggregate) =>
        Try(f(key, value, aggregate)).recover(handler.handle(topic, AggregateOperation, key, value, aggregate)).get
  }
  ...
  @inline private[streams] def toTopic[KR, VR](options: Options[KR, VR]): KTopic[KR, VR] = {
    ...
    val keySerde = requireNonNull(materialized.keySerde, "A serialization format for the record key cannot be null")
    val valueSerde = requireNonNull(materialized.valueSerde, "A serialization format for the record value cannot be null")
    KTopic(keySerde, valueSerde)
  }

  /** Returns the materialization options. */
  @inline private[streams] def storeOptions[K, V](keySerde: KeySerde[K], valueSerde: ValueSerde[V]) =
